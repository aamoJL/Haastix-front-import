    ...
    <Stack alignItems="center" justifyContent="center" spacing={1}>
          ...
          {unReviewedSubmissions.length > 0 && 
            <Dialog open={unReviewedSubmissions.length > 0 ? true : false}>
              <Stack alignItems="center" spacing={1} p={1}>
                ...
                <Typography variant="body1" component="p">{translation.texts.challenge}: {unReviewedSubmissions[0].taskDescription}</Typography>
      ...
      {!isGameMaster && !timeIsUp &&
        <>
          ...
          {showCamera && <ChallengeRoomCamera taskId={roomInfo.details.challengeTasks[currentTaskNumber - 1].taskId} onSubmit={() => {setPlayerWaitingReview(true); setShowCamera(false)}} open={showCamera} close={() => setShowCamera(false)}/>}
          <Scoreboard socket={socket} scores={scores}/>
        </>}
